...
class XConv(torch.nn.Module):
    ...
    def __init__(self, setting, idx, K, dilation, P, C, C_pts_fts, depth_multiplier, with_X_transfomation,
        ...
        self.add_module(name='BN1', module=torch.nn.BatchNorm1d(self.C_pts_fts))
        self.add_module(name='dense2',
                        module=torch.nn.Sequential(torch.nn.Linear(self.C_pts_fts, self.C_pts_fts), torch.nn.ELU()))
        self.add_module(name='BN2', module=torch.nn.BatchNorm1d(self.C_pts_fts))
    ...
    def reset_parameters(self):
        reset(self._modules['dense1'])
        reset(self._modules['dense2'])
        reset(self._modules['BN1'])
        reset(self._modules['BN2'])
        reset(self._modules["x_trans_conv1"])
        reset(self._modules['x_trans_depthConv1'])
        reset(self._modules['x_trans_depthConv2'])
        reset(self._modules['fts_conv'])
        reset(self._modules['fts_conv_BN'])
        reset(self._modules['dense3'])
    ...
    def forward(self, pts, fts, qrs):
        ...
        nn_pts_local = nn_pts - nn_pts_center  # (N, P, K, 3) # relative coordinates

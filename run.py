...
from data_class import ModelNet40, ShapeNetPart,Cifar10,S3DIS,ModelNet10,Mnist,ScanNet
...
def train_process():
    ...
    if config.dataset=="S3DIS":
        scene_loader = DataLoader(scene_set, batch_size=config.validation.batch_size, shuffle=False,
                              num_workers=config.num_workers,  drop_last=False)
    ...
    if config.train.optimizer == 'SGD':
        ...
        optimizer = optim.Adam(net.parameters(), lr=config.train.learning_rate_base,eps=1e-08,weight_decay=1e-4)
    else:
        raise NotImplementedError
    ...
    if config.train.resume:
        if not config.task == "seg":
            ...
            start_epoch = model_recorder.resume(net.module, optimizer, from_measurement='iou')
    ...
    for epoch in range(start_epoch, config.train.num_epochs):
        lr=config.train.learning_rate_base*(math.pow(config.train.decay_rate,epoch//10))
            ...
            with torch.no_grad():
                if config.task == "seg":
                  ...
                  if config.dataset=="ScanNet":
                    ...
                    data_labels_seg=data_h5['label_seg'][...].astype(np.int64)
                    data_h5.close()
                    ...
                    for batch_idx in range(batch_num):
                        ...
                        input_points=scene_tfs((points_batch[indices_shuffle]).reshape((batch_size, sample_num, -1))).to(config.device)
                        
                        seg_probs=net.forward(input_points)
                        probs_2d = np.reshape(seg_probs.detach().cpu().numpy()  , (sample_num * batch_size, -1))
                    ...
                    print(metrics.classification_report(np.concatenate(y_true, axis=0), np.concatenate(y_pred, axis=0))) 
                else :
                    validation_loss,acc = evaluate(valid_loader, net)
                    summary_writer.add_scalar('Validation Accuracy', acc, global_step=epoch)
                    summary_writer.add_scalar('Validation Loss', validation_loss, global_step=epoch)
            if config.task == "seg":      
                model_recorder.add(epoch, net, dict(acc=validation_acc,iou=val_ious,avg_acc=avg_per_class_acc))
            else:
...
def create_model(base_model, ckpt_file=None, from_measurement=None):
    ...
    elif base_model == 'scannet_x8_2048_fps':
        net = PointCNN.scannet_x8_2048_fps4()

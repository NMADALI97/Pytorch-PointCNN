...
def room2blocks(input_path,output_path,max_point_num=8192,args_block_size=1.5,args_grid_size=0.03):
    ...
    data = np.zeros((batch_size, max_point_num, input_data.shape[1]-1))
    xyzrgb=input_data[:,:-1]
    ...
    offsets = [('zero', 0.0), ('half', args_block_size / 2)]
    for offset_name, offset in offsets:
                ...
                xyz_max = np.amax(xyz, axis=0, keepdims=True)
                block_size = (args_block_size, args_block_size, 2 * (xyz_max[0, -1] - xyz_min[0, -1]))
                ...
                print('{} is split into {} blocks.'.format(datetime.now(), blocks.shape[0]))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    xyz_grids = np.floor((block_points - block_min) / args_grid_size).astype(np.int)
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    if xyzrgb.shape[1] >3:
                        block_xzyrgb = np.concatenate([x, z, y, block_rgb], axis=-1)
                         ...
                         block_xzyrgb = np.concatenate([x, z, y], axis=-1)
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        label[idx_in_batch] =-1 # won't be used...
                        ...
                        if ((idx + 1) % batch_size == 0) or \
                            ...
                            filename_h5 = output_path+ '%s_%d.h5' % (offset_name, idx_h5)
...
"""
def scannet_download():       
    BASE_DIR = os.path.dirname(os.path.abspath("__file__"))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
    if not os.path.exists(os.path.join(DATA_DIR, 'scannet')):
        
        www = 'https://drive.google.com/uc?id=12JtCXb436uiIN8s9j_b89-CVXgCuSCBz'
        zipfile = "data.zip"
...
def s3dis_download():       
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'indoor3d_sem_seg_hdf5_data')):
        ...
        os.system('gdown  %s; unzip %s -d data' % (www, zipfile))
        os.system('rm %s' % (zipfile))
        os.system('gdown --id 1Quqo_lDKEX_M0kuJO8ZGEUetz796nSun ; mv zero_0.h5  data/zero_0.h5')
        
        X_train, X_test = train_test_split(glob.glob(os.path.join("data/indoor3d_sem_seg_hdf5_data", '*.h5')) , test_size=0.3) 
...
def load_data_s3dis(partition):
    ...
    for h5_name in file:
        ...
        for i in range(xyz.shape[0]):
            ...
            tmp1.append(pc[choices,:3])
            tmp2.append((fts[i])[choices])
        points.append(np.array(tmp1,dtype=np.float32).reshape((xyz.shape[0],2048,3)))
...
def load_data_scene(h5_name):
    data = h5py.File(h5_name, 'r+')
    
    xyz=(data['data'][...].astype(np.float32))[:,:,:3]
    fts=data['label_seg'][...].astype(np.int64)
    data_num=data['data_num'][...].astype(np.int64)
    return (xyz.reshape(-1,2048,3), None, data_num.reshape(-1), fts.reshape(-1,2048), None)


def load_data_scannet(partition):
    scannet_download()
    BASE_DIR = os.path.dirname(os.path.abspath("__file__"))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    points = []
    ...
    indices_split_to_full = []
    if partition == 'train':
        file=["data/scannet/train/"+u for u in os.listdir("data/scannet/train") if ".h5" in u  ]
    else:
        file=["data/scannet/test/"+u for u in os.listdir("data/scannet/test") if ".h5" in u  ]
    ...
    print("point_nums:",point_nums.min(),"point_nums:",point_nums.max())
    return (points, None, point_nums, labels_seg, None)
...
class ScanNet(Dataset):
    def __init__(self, partition='train',transforms=None):
        ...
        self.points, _, self.point_nums, self.labels_seg, _  = load_data_scannet(partition)
        #self.points=torch.Tensor(pf.global_norm(self.points))
        self.sample_num=2048
        self.transforms=transforms
        print(self.points.shape)
        print(self.labels_seg.shape)
        print(self.point_nums.shape)

      

    def __getitem__(self, index):

        pc=self.points[index]
        fts=self.labels_seg[index]
        pool_size=self.point_nums[index]
        
        if pool_size > self.sample_num:
            choices = np.random.choice(pool_size, self.sample_num, replace=False)
        else:
            choices = np.concatenate((np.random.choice(pool_size, pool_size, replace=False),
                                      np.random.choice(pool_size, self.sample_num - pool_size, replace=True)))
        
        if not self.transforms  is None:
             return self.transforms(pc[choices,:]), fts[choices], -1,-1
        else :
            return pc[choices,:],fts[choices], -1,-1
...
class S3DIS(Dataset):
    def __init__(self, partition='train',transforms=None):
        ...
        if partition in ["train","test"]:
            ...
            self.points, _, self.point_nums, self.labels_seg, _  = load_data_scene(partition)
        #self.points=torch.Tensor(pf.global_norm(self.points))
        self.transforms=transforms
    ...
    def __getitem__(self, index):
        if not self.transforms  is None:
            return self.transforms(self.points[index]), self.labels_seg[index], -1,-1
        else :
             return self.points[index], self.labels_seg[index], -1,-1
